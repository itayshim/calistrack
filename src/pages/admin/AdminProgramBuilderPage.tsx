...
export function AdminProgramEditorPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl pb-28">
                      ...
                      {workout.sections.map((section, si) => (
                        ...
                        >
                          ...
                          {section.exercises.map((item, ei) => (
                            <div className="mt-3 grid gap-2 rounded-2xl bg-slate-50 p-3 md:grid-cols-6 dark:bg-white/[.03]" key={item.key}>
                              <Select
                              ...
                              <label className="field-label md:col-span-3">{l('Technique cue', 'דגש טכני')}<input className="input mt-1" value={item.techniqueCue ?? ''} onChange={(e)=>mutate((d)=>{d.weeks[wi].workouts[di].sections[si].exercises[ei].techniqueCue=e.target.value})}/></label>
                              <label className="field-label md:col-span-3">{l('Technique cue · Hebrew', 'דגש טכני · עברית')}<input className="input mt-1" dir="rtl" value={item.techniqueCueHe ?? ''} onChange={(e)=>mutate((d)=>{d.weeks[wi].workouts[di].sections[si].exercises[ei].techniqueCueHe=e.target.value})}/></label>
                              <label className="field-label">RIR min<input className="input mt-1" type="number" min="0" max="10" value={item.rirMin ?? ''} onChange={(e)=>mutate((d)=>{d.weeks[wi].workouts[di].sections[si].exercises[ei].rirMin=e.target.value===''?undefined:Number(e.target.value)})}/></label>
                              <label className="field-label">RIR max<input className="input mt-1" type="number" min="0" max="10" value={item.rirMax ?? ''} onChange={(e)=>mutate((d)=>{d.weeks[wi].workouts[di].sections[si].exercises[ei].rirMax=e.target.value===''?undefined:Number(e.target.value)})}/></label>
                              {item.progressionRule && <><label className="field-label">{l('Progress minimum', 'מינימום להתקדמות')}<input className="input mt-1" type="number" value={item.progressionRule.minimumAcrossAllSets} onChange={(e)=>mutate((d)=>{d.weeks[wi].workouts[di].sections[si].exercises[ei].progressionRule!.minimumAcrossAllSets=Number(e.target.value)})}/></label><label className="field-label">{l('Progress maximum', 'מקסימום להתקדמות')}<input className="input mt-1" type="number" value={item.progressionRule.maximumAcrossAllSets} onChange={(e)=>mutate((d)=>{d.weeks[wi].workouts[di].sections[si].exercises[ei].progressionRule!.maximumAcrossAllSets=Number(e.target.value)})}/></label></>}
                            </div>
                          ))}
                          <button
      ...
      {definition.milestones?.length ? <section className="card mt-6"><h2 className="section-title">{l('Milestones', 'אבני דרך')}</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{definition.milestones.map((milestone,index)=><div className="surface-subtle rounded-2xl p-3" key={milestone.key}><input className="input font-black" value={language==='he'?milestone.nameHe:milestone.nameEn} onChange={(e)=>mutate((d)=>{if(language==='he')d.milestones![index].nameHe=e.target.value;else d.milestones![index].nameEn=e.target.value})}/><div className="mt-2 grid grid-cols-2 gap-2"><label className="field-label">{l('Threshold','סף')}<input className="input mt-1" type="number" value={milestone.threshold} onChange={(e)=>mutate((d)=>{d.milestones![index].threshold=Number(e.target.value)})}/></label><label className="field-label">{l('Sets','סטים')}<input className="input mt-1" type="number" min="1" value={milestone.setsRequired??1} onChange={(e)=>mutate((d)=>{d.milestones![index].setsRequired=Number(e.target.value)})}/></label></div></div>)}</div></section>:null}
      <section
...
function PrescriptionMobile(props:RowProps){const v=rowValues(props);return <article className="rounded-2xl bg-slate-50 p-3 dark:bg-white/[.04]"><strong>{props.index+1}. {v.name}</strong><code className="mt-1 block text-xs text-slate-400">{props.prescription.exerciseKey}</code><dl className="mt-3 grid grid-cols-2 gap-2 text-sm">{v.labels.map((label,index)=><div key={label}><dt className="text-xs text-slate-500">{label}</dt><dd className="font-bold">{v.values[index]}</dd></div>)}</dl>{props.prescription.progressionRule&&<p className="mt-3 text-xs text-slate-500">{props.language==='he'?props.prescription.progressionRule.guidanceHe:props.prescription.progressionRule.guidanceEn}</p>}</article>}
