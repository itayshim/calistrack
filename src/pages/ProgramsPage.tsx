    ...
    <div className="space-y-8">
      <header className="flex items-end justify-between">
        <div>
          ...
          <h1 className="mt-2 text-4xl font-black tracking-[-.05em]">Program</h1>
          <p className="mt-2 text-slate-400">Train with structure. Progress with purpose.</p>
        </div>
        <Link
      ...
      {!programs.length ? (
        <>
          <section className="relative overflow-hidden rounded-4xl bg-brand p-7 text-ink shadow-glow">
            ...
            <h2 className="mt-6 text-4xl font-black tracking-[-.05em]">{beginnerProgram.name}</h2>
            <p className="mt-3 max-w-lg font-semibold opacity-70">
              Three balanced full-body sessions designed to build your base without overwhelming
              you.
            </p>
          ...
          <EmptyState
            ...
            title="Prefer your own structure?"
            description="Build a training week around your goals, schedule, and favorite movements."
            action="Create a custom program"
        ...
        programs.map((program) => (
          <section key={program.id}>
            <div className="mb-4 flex items-center justify-between">
              <div>
                <Badge tone="brand">ACTIVE PROGRAM</Badge>
            ...
            >
              Edit training week <ChevronRight size={16} />
            </Link>
          </section>
        ))
      )}
