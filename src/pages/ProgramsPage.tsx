  ...
  return (
    <div className="space-y-8">
      <header className="flex items-end justify-between gap-3">
        ...
        <Link
          data-tour-id="create-program-action"
          aria-label={t('createProgram')}
          to="/program/new"
          className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-brand text-ink shadow-glow"
        >
          <Plus size={26} />
        </Link>
      </header>
      ...
      {managedPrograms.length > 0 && (
        <section>
          <div className="mb-4">
            <p className="eyebrow">{t('recommendedStart')}</p>
            <h2 className="text-2xl font-black">
              {language === 'he' ? 'קטלוג תוכניות' : 'Program catalogue'}
            </h2>
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            {managedPrograms.map(({ definition }) => (
              <Link
                className="card transition hover:-translate-y-0.5"
                key={definition.key}
                to={`/programs/managed/${definition.key}`}
              >
                <Badge tone="brand">
                  {definition.durationWeeks} {language === 'he' ? 'שבועות' : 'weeks'} ·{' '}
                  {definition.sessionsPerWeek}/{language === 'he' ? 'שבוע' : 'week'}
                </Badge>
      ...
      {!programs.length ? (
        <>
          <section className="relative overflow-hidden rounded-4xl bg-brand p-7 text-ink shadow-glow">
            ...
            <h2 className="mt-6 text-4xl font-black tracking-[-.05em]">
              {t('beginnerProgramName')}
            </h2>
            <p className="mt-3 max-w-lg font-semibold opacity-70">
              {t('beginnerProgramDescription')}
            </p>
            <button
        ...
        programs.map((program) => (
          <section key={program.id}>
            <div
              data-tour-id={program.id === activeProgramId ? 'active-program-summary' : undefined}
            ...
            >
              <div className="min-w-0">
                ...
                <h2 className="mt-2 break-words text-3xl font-black tracking-[-.04em]">
                  {program.name}
                </h2>
              </div>
              <ActionMenu
                ...
                items={[
                  {
                    id: 'edit',
                    label: t('editProgramAction'),
                    icon: <Pencil size={19} />,
                    onSelect: () => nav(`/program/${program.id}`),
                  },
                  {
                    id: 'rename',
                    label: t('renameProgram'),
                    icon: <Pencil size={19} />,
                    onSelect: () => openRename(program),
                  },
                  {
                    id: 'duplicate',
                    label: t('duplicateProgram'),
                    icon: <Copy size={19} />,
                    onSelect: () => duplicateProgram(program.id),
                  },
                  ...(program.id !== activeProgramId
                    ? [
                        {
                          id: 'activate',
                          label: t('setActiveProgram'),
                  ...
                  <div className="flex w-full items-center gap-3 p-5 text-start sm:gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 flex flex-wrap items-center gap-2 text-sm font-semibold text-slate-400">
                        ...
                        <span>
                          <bdi>{workout.exercises.length}</bdi> {t('exercisesCount')}
                        </span>
                      </p>
                      <div className="mt-3">
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      ...
                      <button
                        type="button"
                        title={(activeWorkout?.workoutTemplateId === workout.id
                          ? t('resumeWorkoutNamed')
                          : t('startWorkoutNamed')
                        ).replace('{name}', workout.name)}
                        aria-label={(activeWorkout?.workoutTemplateId === workout.id
                          ? t('resumeWorkoutNamed')
                          : t('startWorkoutNamed')
                        ).replace('{name}', workout.name)}
      ...
      {renameTarget && (
        <div
          ...
          role="presentation"
          onMouseDown={(event) => event.target === event.currentTarget && setRenameTarget(null)}
        >
          <section
            role="dialog"
            aria-modal="true"
            aria-labelledby="rename-program-title"
            className="modal-surface w-full max-w-sm rounded-4xl p-6"
          >
            <h2 id="rename-program-title" className="text-2xl font-black">
              {t('renameProgramTitle')}
            </h2>
            <label className="mt-5 block">
              ...
              />
              {!renameValue.trim() && (
                <span className="mt-2 block text-sm font-bold text-red-500">
                  {t('programNameRequired')}
                </span>
              )}
            </label>
      ...
      <ConfirmDialog
        ...
        description={
          deleteTarget?.id === activeProgramId
            ? t('deleteActiveProgramDescription')
            : t('deleteProgramDescription')
        }
